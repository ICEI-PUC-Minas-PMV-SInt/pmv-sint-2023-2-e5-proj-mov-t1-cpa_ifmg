...
namespace api_cpa_ifmg.Controllers
{
    ...
    public class CronogramasController : ControllerBase
    {
        ...
        {
            var model = await _context.Cronogramas.ToListAsync();
            return Ok(model);
        }
        [HttpPost]
        public async Task<ActionResult> Create(Cronograma model)
        {
        ...
        [HttpPut("{id}")]
        public async Task<ActionResult> Update(int id, Cronograma model)
        {
            if (id != model.Id) return BadRequest();
            var modelDb = await _context.Cronogramas.AsNoTracking()
                .FirstOrDefaultAsync(c => c.Id == id);

            if (modelDb == null) return NotFound();

            _context.Cronogramas.Update(model);
            await _context.SaveChangesAsync();
